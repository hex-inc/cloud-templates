...
resource "newrelic_infra_alert_condition" "high_cpu_usage" {
  ...
  name       = "High cpu usage in hex-chime"
  type       = "infra_metric"
  event      = "K8sNodeSample"
  select     = "allocatableCpuCoresUtilization"
  comparison = "above"
  where      = "(clusterName = '${local.monitoring_cluster_name}')"
...
resource "newrelic_infra_alert_condition" "high_disk_usage" {
  ...
  name       = "High disk usage in hex-chime"
  type       = "infra_metric"
  event      = "K8sNodeSample"
...
resource "newrelic_infra_alert_condition" "high_memory_usage" {
  ...
  name       = "High memory usage in ${local.monitoring_cluster_name}"
  type       = "infra_metric"
  event      = "K8sNodeSample"
...
resource "newrelic_infra_alert_condition" "host_not_reporting" {
  ...
  name  = "Host not reporting in ${local.monitoring_cluster_name}"
...
resource "newrelic_infra_alert_condition" "pod_capacity_low" {
  ...
  name       = "Low pod capacity in ${local.monitoring_cluster_name}"
  type       = "infra_metric"
  event      = "K8sNodeSample"
  select     = "capacityPods"
  comparison = "below"
...
resource "newrelic_infra_alert_condition" "container_memory_utilization" {
  ...
  name       = "Container memory utilization high in ${local.monitoring_cluster_name}"
  type       = "infra_metric"
  event      = "K8sContainerSample"
  select     = "memoryUtilization"
  comparison = "above"
  where      = "(clusterName = '${local.monitoring_cluster_name}' AND containerName NOT LIKE 'hex-python-kernel')"
...
resource "newrelic_infra_alert_condition" "container_cpu_utilization" {
  ...
  name       = "Container cpu utilization high in ${local.monitoring_cluster_name}"
  type       = "infra_metric"
  event      = "K8sContainerSample"
...
resource "newrelic_infra_alert_condition" "container_fs_utilization" {
  ...
  name       = "Container fs utilization high in ${local.monitoring_cluster_name}"
  type       = "infra_metric"
  event      = "K8sContainerSample"
...
resource "newrelic_infra_alert_condition" "deployment_container_restarts" {
  ...
  name       = "Containers restarting ${each.key} in cluster ${local.monitoring_cluster_name}"
  type       = "infra_metric"
  event      = "K8sContainerSample"
  select     = "restartCount"
  comparison = "above"
  where      = "(clusterName = '${local.monitoring_cluster_name}' and deploymentName = '${each.key}')"
...
resource "newrelic_nrql_alert_condition" "low_available_kernels" {
  ...
  type                         = "static"
  name                         = "Low available kernels in ${each.key} in cluster ${local.monitoring_cluster_name}"
  enabled                      = true
  ...
  nrql {
    query             = "FROM K8sDeploymentSample SELECT average(podsAvailable / podsDesired * 100) WHERE (clusterName = '${local.monitoring_cluster_name}' and deploymentName = '${each.key}')"
    evaluation_offset = 3
  }
...
resource "newrelic_nrql_alert_condition" "stateful_set_pod_missing" {
  ...
  name = "Pod missing in ${each.key} in cluster ${local.monitoring_cluster_name}"
  type = "static"
  ...
  nrql {
    query             = "FROM K8sStatefulsetSample SELECT latest(podsMissing) WHERE (clusterName = '${local.monitoring_cluster_name}' and statefulsetName = '${each.key}')"
    evaluation_offset = 3
  }
...
resource "newrelic_nrql_alert_condition" "hex_deployment_pods_unavailable" {
  ...
  name = "Pods unavailable in ${var.name}-hex in cluster ${local.monitoring_cluster_name}"
  type = "static"
  ...
  nrql {
    query             = "FROM K8sDeploymentSample SELECT max(podsDesired - podsAvailable) WHERE (clusterName = '${local.monitoring_cluster_name}' and deploymentName = '${var.name}-hex')"
    evaluation_offset = 3
  }
...
resource "newrelic_nrql_alert_condition" "deployment_pods_unavailable" {
  ...
  name = "Pods unavailable in ${each.key} in cluster ${local.monitoring_cluster_name}"
  type = "static"
  ...
  nrql {
    query             = "FROM K8sDeploymentSample SELECT max(podsDesired - podsAvailable) WHERE (clusterName = '${local.monitoring_cluster_name}' and deploymentName = '${each.key}')"
    evaluation_offset = 3
  }
...
resource "newrelic_nrql_alert_condition" "vault_leader_last_contact" {
  ...
  type                         = "static"
  name                         = "Vault leader last contact high in ${local.monitoring_cluster_name}"
...
resource "newrelic_nrql_alert_condition" "vault_sealed" {
  ...
  type                         = "static"
  name                         = "Vault sealed in ${local.monitoring_cluster_name}"
...
resource "newrelic_nrql_alert_condition" "high_average_node_heap_usage" {
  ...
  type                         = "static"
  name                         = "Average node heap usage high in '${var.name}-hex' in cluster ${local.monitoring_cluster_name}"
...
resource "newrelic_nrql_alert_condition" "high_max_node_heap_usage" {
  ...
  type                         = "static"
  name                         = "Node heap usage spiked in '${var.name}-hex' in cluster ${local.monitoring_cluster_name}"
